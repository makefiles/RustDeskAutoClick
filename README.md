# RustDesk Auto-Accept

> ⚠️ **은퇴 (Retired, 2026-07-13)**
>
> RustDesk가 **영구 비밀번호(무인 접속, unattended access)** 를 지원하면서, 이 도구가 대신하던 "수락 버튼 자동 클릭"이 더 이상 필요 없어졌습니다. 무인 접속을 켜면 비밀번호만 맞으면 수락 창 없이 바로 연결됩니다.
>
> **대체 방법:** RustDesk 설정 → 보안(Security) → 영구 비밀번호 지정 (필요 시 "연결 시 수동 확인" 옵션 끄기)
>
> 이 저장소는 기록 보존용으로 남기며 더 이상 사용·유지보수하지 않습니다. 현재 PC의 systemd 서비스는 제거되었고, 필요하면 `./install_linux.sh`로 재설치할 수 있습니다. 아래 문서는 은퇴 이전 기준입니다.

RustDesk 연결 요청이 들어오면 자동으로 "수락" 버튼을 클릭해주는 프로그램입니다.

RustDesk 유료 기능인 자동 수락을 무료로 구현합니다. Linux와 Windows 모두 지원합니다.

## 빠른 시작

### 1. config.json 설정

먼저 예시 파일을 복사합니다:

```bash
cp config.example.json config.json      # Windows: copy config.example.json config.json
```

그다음 `config.json`을 편집합니다. 모든 연결을 자동 수락하려면:

```json
{
  "mode": "allow_all"
}
```

특정 PC만 허용하려면:

```json
{
  "mode": "whitelist",
  "allowed_ids": ["123456789", "987654321"]
}
```

`allowed_ids`에 허용할 RustDesk ID를 넣으세요. (RustDesk 메인 화면 상단의 숫자, 공백 없이 숫자만)

> `config.json`은 개인 ID가 들어가므로 `.gitignore`에 등록되어 버전 관리에서 제외됩니다. 템플릿은 `config.example.json`을 사용하세요.

### 2. 실행

```bash
python3 rustdesk_autoclick.py
```

### 3. (선택) 부팅 시 자동 실행

**Linux:**
```bash
pip install python-xlib
sudo apt install xdotool   # Ubuntu/Debian
./install_linux.sh
```

**Windows:**
```
install_windows.bat
```
관리자 권한을 자동으로 요청합니다.

## 제거

**Linux:** `./uninstall_linux.sh`

**Windows:** `uninstall_windows.bat` 실행

## 설정 상세

`config.json` 전체 옵션:

```json
{
  "mode": "whitelist",
  "allowed_ids": ["YOUR_RUSTDESK_ID_HERE"],
  "dialog_size": {
    "width": 300,
    "height": 490,
    "tolerance": 50
  },
  "button_position": {
    "x_ratio": 0.25,
    "y_ratio": 0.95
  },
  "click_delay": 0.5,
  "idle_threshold": 1.0,
  "idle_timeout": 30.0,
  "log_file": "./autoclick.log"
}
```

| 항목 | 기본값 | 설명 |
|------|--------|------|
| `mode` | `whitelist` | `"whitelist"` = ID 기반 허용, `"allow_all"` = 모두 수락 |
| `allowed_ids` | — | 허용할 RustDesk ID 목록 (whitelist 모드 전용) |
| `dialog_size` | 300x490 | 연결 요청 다이얼로그 크기, `tolerance`는 허용 오차 |
| `button_position` | 0.25, 0.95 | 수락 버튼 위치 (창 크기 대비 비율) |
| `click_delay` | 0.5 | 감지 후 클릭 대기 시간 (초) |
| `idle_threshold` | 1.0 | 마우스가 이 시간동안 멈춰야 클릭 (초) |
| `idle_timeout` | 30.0 | 이 시간이 지나면 마우스 상태와 관계없이 클릭 (초) |
| `log_file` | ./autoclick.log | 로그 파일 경로 |

> 대부분의 경우 `mode`와 `allowed_ids`만 설정하면 됩니다. 나머지는 기본값으로 동작합니다.

## 요구사항

- Python 3.8+
- **Linux**: `python-xlib`, `xdotool` (X11 환경)
- **Windows**: 추가 설치 없음

## 동작 원리

1. RustDesk 프로세스의 창을 **프로세스명 + 크기 + 타이틀 패턴**으로 감지
2. OS 이벤트 + 2초 주기 스캔으로 이중 감지
3. 마우스가 움직이지 않을 때 수락 버튼 클릭
4. 동일 peer의 연결 상태 창은 쿨다운으로 무시

> **참고:** 첫 시도에서 수락이 안 될 경우, 연결 요청 측에서 취소 후 다시 시도해주세요.

## 제한사항

- **화면 잠금 시 동작하지 않습니다.** 화면이 잠기면 OS가 입력 이벤트를 잠금 화면으로 가로채기 때문에 RustDesk 다이얼로그에 클릭이 전달되지 않습니다. 화면이 잠금 해제된 상태에서만 동작합니다.
- **Linux:** X11 환경만 지원합니다. Wayland는 지원하지 않습니다.

## 라이선스

MIT
